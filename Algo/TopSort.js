...
{		
	this.startButton = addControlToAlgorithmBar("Button", "Сортировать");
...
TopoSortDFS.prototype.init = function(am, w, h)
{
	...
	TopoSortDFS.superclass.init.call(this, am, w, h, true, true); 
}
...
{		
	...
	this.cmd("CreateLabel", headerID, "Топологический порядок",  TopoSortDFS.ORDERING_INITIAL_X, TopoSortDFS.ORDERING_INITIAL_Y - 1.5*TopoSortDFS.ORDERING_DELTA_Y);
		...
		for (var neighbor = 0; neighbor < this.size; neighbor++)
		{
			if (this.adj_matrix[startVertex][neighbor] > 0)
			{
				...
				if (!this.visited[neighbor])
				{
					...
					nextMessage = this.nextIndex;
					this.cmd("CreateLabel", nextMessage, "Выход из рекурсивной функции: DFS(" + String(neighbor) + ")", messageX + 20, this.messageY, 0);
